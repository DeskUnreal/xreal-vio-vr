...
{
class HMDDevice : public vr::IVRDisplayComponent,
	...
	virtual void *
	GetComponent(const char *pchComponentNameAndVersion) override;
	...
	virtual void
	GetRecommendedRenderTargetSize(uint32_t *pnWidth,
				       uint32_t *pnHeight) override;
	virtual void GetEyeOutputViewport(vr::EVREye eEye, uint32_t *pnX,
					  uint32_t *pnY, uint32_t *pnWidth,
					  uint32_t *pnHeight) override;
	virtual void GetProjectionRaw(vr::EVREye eEye, float *pfLeft,
	...
	virtual bool ComputeInverseDistortion(vr::HmdVector2_t *pResult,
					      vr::EVREye eEye,
