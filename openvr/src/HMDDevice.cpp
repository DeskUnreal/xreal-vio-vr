...
HMDDevice::HMDDevice(vr::IVRDriverContext *pDriverContext)
	: last_pose()
{
...
void HMDDevice::RunFrame()
{
	...
	driver_host->TrackedDevicePoseUpdated(deviceId, last_pose,
					      sizeof(last_pose));
}
...
void *HMDDevice::GetComponent(const char *pchComponentNameAndVersion)
{
	...
	if (!strcmp(pchComponentNameAndVersion,
...
/* IVRDisplayComponent */
void HMDDevice::GetWindowBounds(int32_t *pnX, int32_t *pnY, uint32_t *pnWidth,
				uint32_t *pnHeight)
{
	*pnX = 0;
	*pnY = 0;
...
void HMDDevice::GetRecommendedRenderTargetSize(uint32_t *pnWidth,
					       uint32_t *pnHeight)
{
	*pnWidth = 2 * 1920;
	*pnHeight = 1080;
...
void HMDDevice::GetEyeOutputViewport(vr::EVREye eEye, uint32_t *pnX,
...
void HMDDevice::GetProjectionRaw(vr::EVREye eEye, float *pfLeft, float *pfRight,
 ...
 */
vr::DistortionCoordinates_t HMDDevice::ComputeDistortion(vr::EVREye eEye,
							 float fU, float fV)
{
...
bool HMDDevice::ComputeInverseDistortion(vr::HmdVector2_t *pResult,
